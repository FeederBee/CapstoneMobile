...
@onready var walk_timer: Timer = $Timer
...
func _ready() -> void:
	...
	_set_random_target()
	  ...
	  # Setup timer untuk durasi maksimal berjalan
	walk_timer.one_shot = true  # Timer hanya berjalan sekali
	walk_timer.connect("timeout", Callable(self, "_stop_walking"))
...
func _move_towards_target(delta: float) -> void:
	...
	# Jika sudah mencapai target atau waktu habis, berhenti dan atur target baru setelah jeda
	if position.distance_to(target_position) < 5 or walk_timer.time_left == 0:
		_stop_walking()

# --- Fungsi untuk Menghentikan Gerakan ---
func _stop_walking() -> void:
	# Fungsi untuk menghentikan anjing dari berjalan, baik karena mencapai target atau waktu habis
	is_moving = false
	_play_idle_animation()
	_wait_and_set_new_target()

func _set_timer():
	# Mulai timer maksimal berjalan di detik
	walk_timer.wait_time = 3.0
	walk_timer.start()
